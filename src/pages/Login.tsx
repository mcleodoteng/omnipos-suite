      ...
      <div className="w-full max-w-md animate-fade-in relative z-10">
        ...
        <div className="glass rounded-2xl p-6 space-y-6">
          <div className="space-y-3">
            ...
            {users.length === 0 && (
              <p className="text-sm text-muted-foreground text-center py-4">No users found. Contact administrator.</p>
            )}
          </div>
              ...
              {[0, 1, 2, 3].map(i => (
                <div
                  ...
                  className={`w-14 h-14 rounded-xl border-2 flex items-center justify-center text-2xl font-bold transition-all duration-200 ${
                    pin.length > i ? 'border-primary bg-primary/10 text-primary' : 'border-border bg-card'
                  }`}
                >
                  {pin.length > i ? '•' : ''}
                </div>
